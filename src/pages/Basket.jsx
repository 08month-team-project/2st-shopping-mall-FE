...
const Basket = () => {
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      await axios.patch(
        `http://ec2-3-35-135-46.ap-northeast-2.compute.amazonaws.com:8080/carts/items/${id}?quantity=${item.newQuantity}`,
        {}
      );
      setItems((prevItems) =>
        prevItems.map((item) =>
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `http://ec2-3-35-135-46.ap-northeast-2.compute.amazonaws.com:8080/carts/items?cart_item_id=${id}`,
        {
          method: 'DELETE',
        }
      );
